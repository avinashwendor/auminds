...
export default function LoginPage() {
  ...
  return (
    <main className="grid min-h-screen bg-background lg:grid-cols-[.8fr_1.2fr]">
      <section className="flex flex-col justify-between border-b border-border bg-card p-6 sm:p-10 lg:border-b-0 lg:border-r">
        <Link href="/" className="inline-flex w-fit items-center gap-2 text-sm text-muted-foreground hover:text-foreground">
          <ArrowLeft className="size-4" /> Back to AUMINDS
        </Link>
        <div className="my-16 max-w-lg">
          <div className="mb-6 flex items-center gap-3">
            <span className="signal-dot" />
            <span className="board-label">Access control · online</span>
          </div>
          <h1 className="board-value text-5xl leading-[.94] sm:text-7xl">YOUR WORKSPACE IS ONE STOP AWAY.</h1>
          <p className="mt-6 max-w-md text-lg text-muted-foreground">Continue lessons, run code, submit projects, or manage the academy network.</p>
        </div>
      ...
      <section className="flex items-center justify-center p-5 sm:p-10">
        <div className="w-full max-w-md">
          <div className="mb-8">
            <p className="board-label text-primary">Identity check</p>
            <h2 className="board-value mt-3 text-3xl">SIGN IN</h2>
            <p className="mt-2 text-sm text-muted-foreground">Use the credentials assigned to your account.</p>
          </div>
