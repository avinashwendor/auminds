...
function initMxdApp(force = false) {
  if (window.__mxdAppInitialized && !force) return;
  window.__mxdAppInitialized = true;
  ...
  if (deviceType() === "desktop") {
    ...
    const cursorEl = document.getElementById("mxd-cursor");
...
if (document.readyState === "complete" || document.readyState === "interactive") {
  setTimeout(initMxdApp, 100);
} else {
  document.addEventListener("DOMContentLoaded", initMxdApp);
}
window.initMxdApp = initMxdApp;
// --------------------------------------------- //
// Base - Inits End
// --------------------------------------------- //
...
function pageAppearance() {
  ...
  loadingChars.forEach((el) => {
    SplitText.create(el, {
      ...
      aria: "none",
      onSplit: (self) => {
        if (!hasTargets(self.chars)) return;
  ...
  loadingSplits.forEach((el) => {
    SplitText.create(el, {
      ...
      aria: "none",
      onSplit: (self) => {
        if (!hasTargets(self.lines)) return;
...
function mxdTypeAnimations() {
  ...
  splitTypes.forEach((char,i) => {
    ...
    if (!hasTargets(text.chars)) return;
...
function mxdCursor() {
  ...
  if (!mxdCursor || !mxdCursorDot || !mxdCursorText || !mxdCursorImage) return;
  ...
  document.addEventListener("pointermove", (event) => {
    if (mxdCursor) gsap.set(mxdCursor, { x: event.clientX, y: event.clientY });
  });

  window.addEventListener("mousedown", (event) => {
    if (mxdCursor) mxdCursor.style.transform = `translateX(${event.clientX}px) translateY(${event.clientY}px)`;
  });

  document.addEventListener("mouseleave", () => {
    if (mxdCursor) gsap.to(mxdCursor, { duration: 0.4, ease: "power1.in", scale: 0 });
    if (mxdCursorImage) gsap.to(mxdCursorImage, { rotation: 0, duration: 0.3, ease: "power1.in" });
  });
  ...
  activeEl.forEach((e) => {
    e.addEventListener("mouseenter", () => {
      ...
      if (mxdCursorDot) mxdCursorDot.classList.add("active", "expand");
    });
    e.addEventListener("mouseleave", () => {
      if (mxdCursorText) mxdCursorText.classList.remove("show");
      if (mxdCursorDot) mxdCursorDot.classList.remove("active", "expand");
    });
  });
  // permanent cursor
  activeElPermanent.forEach((e) => {
    e.addEventListener("mouseenter", () => {
      ...
      if (mxdCursorDot) mxdCursorDot.classList.add("active-permanent", "expand");
    });
    e.addEventListener("mouseleave", () => {
      if (mxdCursorText) mxdCursorText.classList.remove("show", "permanent");
      if (mxdCursorDot) mxdCursorDot.classList.remove("active-permanent", "expand");
    });
  });
  // accent cursor
  activeElAccent.forEach((e) => {
    e.addEventListener("mouseenter", () => {
      ...
      if (mxdCursorDot) mxdCursorDot.classList.add("active-accent", "expand");
    });
    e.addEventListener("mouseleave", () => {
      if (mxdCursorText) mxdCursorText.classList.remove("show", "accent");
      if (mxdCursorDot) mxdCursorDot.classList.remove("active-accent", "expand");
    });
  });
  // links
  links.forEach((link) => {
    link.addEventListener("mouseenter", () => {
      if (mxdCursor) mxdCursor.classList.add("difference");
      if (mxdCursorDot) mxdCursorDot.classList.add("cursor-btn-link");
    });
    link.addEventListener("mouseleave", () => {
      if (mxdCursor) mxdCursor.classList.remove("difference");
      if (mxdCursorDot) mxdCursorDot.classList.remove("cursor-btn-link");
    });
  });
  // opposite sections
  oppositeBg.forEach(oppositeBg => {
    oppositeBg.addEventListener("mouseover", () => {
      if (mxdCursorDot) mxdCursorDot.classList.add("active-opposite");
    });

    oppositeBg.addEventListener("mouseleave", () => {
      if (mxdCursorDot) mxdCursorDot.classList.remove("active-opposite");
    });
  });
  ...
  activeElImage.forEach((el) => {
    ...
    el.addEventListener("mouseleave", () => {
      ...
      if (mxdCursorDot) mxdCursorDot.classList.remove("expand");
    });
  });
...
function mxdCommonAnimations() {
  ...
  imageContainer.forEach((element) => {
    ...
    if (!hasTargets(element) || !hasTargets(image)) return;
  ...
  animSlideDownWraps.forEach((animSlideDownWrap) => {
    ...
    if (!hasTargets(animSlideDownEL)) return;
...
// --------------------------------------------- //
function mxdParallax() {
  if (typeof Ukiyo === 'undefined') return;
  ...
  if (hasTargets(images)) {
    new Ukiyo(images, { scale: 1.4, speed: 1.5, externalRAF: false });
  }
  if (hasTargets(imagesSmall)) {
    new Ukiyo(imagesSmall, { scale: 1.2, speed: 1.5, externalRAF: false });
  }
  if (hasTargets(video)) {
    new Ukiyo(video, { scale: 1.4, speed: 1.5, externalRAF: false });
  }
}
// --------------------------------------------- //
