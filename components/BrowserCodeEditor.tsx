...
function getRuntime() {
  ...
  if (!runtimeWindow.__aumindsWebContainer) {
    ...
    runtimeWindow.__aumindsWebContainer = bootPromise;
    void bootPromise.catch(() => {
      if (runtimeWindow.__aumindsWebContainer === bootPromise) {
        delete runtimeWindow.__aumindsWebContainer;
      }
    });
  }
...
export default function BrowserCodeEditor() {
  ...
  useEffect(() => {
    ...
    const isolationReloadKey = 'auminds-code-editor-isolation-reload';
  ...
  useEffect(() => {
    window.localStorage.setItem('auminds-editor-layout', JSON.stringify({ explorerWidth, editorShare, terminalShare }));
    ...
    <main className="flex h-screen min-h-[620px] flex-col overflow-hidden bg-[#0B0F17] text-white">
      <header className="flex min-h-14 flex-wrap items-center justify-between gap-3 border-b border-white/10 bg-[#10151d] px-4 py-2">
        ...
        <div className="flex items-center gap-2 text-xs">
          <span className={`size-2 rounded-full ${runtimeStatus === 'ready' ? 'bg-[#00AB55]' : runtimeStatus === 'error' ? 'bg-red-500' : 'animate-pulse bg-amber-400'}`} />
          ...
          <button type="button" onClick={runActiveFile} disabled={runtimeStatus !== 'ready' || !activeFile} className="inline-flex h-9 items-center gap-2 bg-[#00AB55] px-4 font-bold disabled:opacity-40">
      ...
      <section ref={workspaceRef} className="flex min-h-0 flex-1 overflow-hidden">
        ...
        <button
          ...
          className="w-1.5 shrink-0 cursor-col-resize border-x border-white/5 bg-[#151b24] hover:bg-[#00AB55] focus:bg-[#00AB55]"
        />
        ...
        <div ref={mainPaneRef} className="flex min-w-0 flex-1 flex-col overflow-hidden">
          ...
          <button
            ...
            className="h-1.5 shrink-0 cursor-row-resize border-y border-white/5 bg-[#151b24] hover:bg-[#00AB55] focus:bg-[#00AB55]"
          />
            ...
            <div className="flex h-10 shrink-0 items-center justify-between border-b border-white/10 bg-[#10151d] px-3">
              <div role="tablist" aria-label="Workspace tools" className="flex h-full items-center">
                <button
                  ...
                  className={`inline-flex h-full items-center gap-2 border-b-2 px-3 text-xs font-bold uppercase tracking-wider ${bottomTab === 'terminal' ? 'border-[#00AB55] text-white' : 'border-transparent text-[#919EAB] hover:text-white'}`}
                >
                ...
                <button
                  ...
                  className={`inline-flex h-full items-center gap-2 border-b-2 px-3 text-xs font-bold uppercase tracking-wider ${bottomTab === 'api' ? 'border-[#00AB55] text-white' : 'border-transparent text-[#919EAB] hover:text-white'}`}
                >
