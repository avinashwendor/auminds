    ...
    <div className="h-full overflow-auto bg-[#0b1017] p-3 text-xs text-white">
      <div className="flex min-w-[520px] gap-2">
        ...
        <select
          ...
          className="h-9 border border-white/10 bg-[#161c24] px-3 font-mono font-bold text-[#00AB55] outline-none focus:border-[#00AB55]"
        >
        ...
        <input
          ...
          className="h-9 min-w-0 flex-1 border border-white/10 bg-[#090d12] px-3 font-mono text-white outline-none focus:border-[#00AB55]"
        ...
        {loading ? (
          ...
          <button type="button" onClick={() => { void sendRequest(); }} className="inline-flex h-9 items-center gap-2 bg-[#00AB55] px-4 font-bold hover:bg-[#009a4d]">
      ...
      <p className="mt-2 text-[11px] text-[#637381]">
        Requests run from your browser. External APIs must allow CORS; relative URLs target this AUMINDS deployment.
      </p>
      ...
      <div className="mt-3 grid min-w-[720px] grid-cols-2 gap-3">
        <section className="space-y-3" aria-label="Request configuration">
          <label className="block">
            ...
            <textarea
              ...
              spellCheck={false}
              className="h-32 w-full resize-y border border-white/10 bg-[#090d12] p-3 font-mono leading-5 outline-none focus:border-[#00AB55]"
            />
          </label>
          <label className="block">
            ...
            <textarea
              ...
              spellCheck={false}
              className="h-48 w-full resize-y border border-white/10 bg-[#090d12] p-3 font-mono leading-5 outline-none focus:border-[#00AB55] disabled:cursor-not-allowed disabled:opacity-35"
            />
          </label>
        </section>
        ...
        <section className="min-w-0 space-y-3" aria-label="API response" aria-live="polite">
          <div className="flex h-8 items-center gap-3 border-b border-white/10">
            ...
            {result && (
              <>
                <span className={`font-mono font-bold ${result.status >= 200 && result.status < 400 ? 'text-[#00AB55]' : 'text-red-400'}`}>
