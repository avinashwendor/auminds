...
export default function MonacoCodeEditor({
  ...
  const handleRunCode = () => {
    ...
    setTimeout(() => {
      if (mainFile.language === 'javascript' || mainFile.language === 'typescript') {
        ...
        logs.push(`[AUMINDS Python 3.11 WASM Engine] Executing ${mainFile.name}...`);
        logs.push(`----------------------------------------`);
        try {
        ...
        logs.push(`[AUMINDS Compiler Hub] Compiling ${mainFile.name} (${mainFile.language.toUpperCase()})...`);
  ...
  const combinedPreviewDoc = `
    ...
    <html>
      ...
      <body>
        ${htmlFile || '<div id="app" style="font-family: sans-serif; padding: 20px;"><h2 style="color:#6366f1;">AUMINDS Monaco Workspace</h2><p>Your interactive live preview is active.</p></div>'}
        <script>
          try {
            ${jsFile}
          } catch(e) {
